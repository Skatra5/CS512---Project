        ...
        for encoding in encodings:
            try:
                with open(file_path, 'r', encoding=encoding) as f:
                    print(f"Successfully opened {filename} with {encoding} encoding")
                    for line in f:
                        text = preprocess_text(line)
                        words = text.strip().split()
                        word_counter.update(words)
                break  # If successful, move to next file
...
def build_noun_vocab(data_dir, vocab):
    ...
    for filename in ["sentiment.train.0", "sentiment.train.1"]:
        ...
        for encoding in encodings:
            try:
                with open(file_path, 'r', encoding=encoding) as f:
                    print(f"Successfully opened {filename} with {encoding} encoding for nouns")
                    for line in f:
                        text = preprocess_text(line)
                        tokens = text.strip().split()
                        # Simple noun heuristic
                        nouns = [word for word in tokens if word.isalpha() and not word in {
...
# Modify the TextDataset _init_ method
    ...
    def __init__(self, data_dir, vocab, noun_vocab, max_length=MAX_SEQ_LENGTH, split='train'):
        ...
        for filename in files:
            ...
            try:
                ...
                print(f"Successfully loaded {filename} with {len(lines)} lines")
